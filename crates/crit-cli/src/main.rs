...
use crit_core::events::get_agent_identity;
use crit_core::jj::{resolve_crit_root_from_path, resolve_workspace_root};
use crit_core::scm::{resolve_backend, resolve_preference};
...
fn main() -> Result<()> {
    let _telemetry = crit_cli::telemetry::init();
    let cli = Cli::parse();
    ...
    let (crit_root, workspace_root) = if let Some(path) = &cli.path {
        ...
        let crit_root = resolve_crit_root_from_path(path)
            ...
            .unwrap_or_else(|_| {
                crit_core::scm::git::detect_git_root(path).unwrap_or_else(|| path.clone())
            });
        ...
        let crit_root = resolve_crit_root_from_path(&workspace_root)
            ...
            .unwrap_or_else(|_| {
                crit_core::scm::git::detect_git_root(&workspace_root)
    ...
    match cli.command {
        ...
        Commands::Reviews(cmd) => match cmd {
            ...
            ReviewsCommands::List {
                ...
                let status_str = status.map(|s| match s {
                    crit_cli::cli::ReviewStatus::Open => "open",
                    crit_cli::cli::ReviewStatus::Approved => "approved",
                    crit_cli::cli::ReviewStatus::Merged => "merged",
                    crit_cli::cli::ReviewStatus::Abandoned => "abandoned",
                });
        ...
        Commands::Threads(cmd) => match cmd {
            ...
            ThreadsCommands::List {
                ...
                let status_str = status.map(|s| match s {
                    crit_cli::cli::ThreadStatus::Open => "open",
                    crit_cli::cli::ThreadStatus::Resolved => "resolved",
                });
                let since_dt = since
                    .map(|s| crit_cli::cli::commands::reviews::parse_since(&s))
                    .transpose()?;
                run_threads_list(
                    &crit_root,
                    &review_id,
