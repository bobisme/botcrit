...
use crit_core::events::{Event, EventEnvelope};
use crit_core::log::{open_or_create_review, AppendLog, FileLog};
use crate::output::OutputFormat;
use crit_core::version::{detect_version, write_version_file, DataVersion, CURRENT_VERSION};
...
mod tests {
    ...
    use crit_core::log::{list_review_ids, open_or_create};
    ...
    fn test_migrate_v1_to_v2() {
        ...
        let log1 = crit_core::log::ReviewLog::new(crit_root, "cr-001").unwrap();
        let events1 = log1.read_all().unwrap();
        assert_eq!(events1.len(), 4);

        // Check events in cr-002 (ReviewCreated, ThreadCreated, CommentAdded)
        let log2 = crit_core::log::ReviewLog::new(crit_root, "cr-002").unwrap();
    ...
    fn test_migrate_preserves_thread_linked_events() {
        ...
        let review_log = crit_core::log::ReviewLog::new(crit_root, "cr-001").unwrap();
    ...
    fn test_remigrate_from_backup_recovers_comments() {
        ...
        let pre_events = crit_core::log::ReviewLog::new(crit_root, "cr-001")
            .unwrap()
            .read_all()
            .unwrap();
        ...
        let post_events = crit_core::log::ReviewLog::new(crit_root, "cr-001")
            .unwrap()
            .read_all()
            .unwrap();
        assert_eq!(
