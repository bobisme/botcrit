#!/usr/bin/env bash
# Generate a Git-based demo project with realistic seal data.
#
# Usage:
#   ./scripts/generate-demo-git.sh          # Creates demo in /tmp/seal-demo-git-XXXXXX
...
DEMO_DIR="${1:-$(mktemp -d /tmp/seal-demo-git-XXXXXX)}"
...
seal_as "quiet-owl" comment "$R1" --file src/auth.rs --line 43 \
...
T_BUILDER=$(seal_as "swift-falcon" --json comment "$R2" --file src/config.rs --line 22 \
	"Consider using a builder pattern instead of mutating fields." \
	2>/dev/null | extract_id thread_id)
seal_as "bold-tiger" reply "$T_BUILDER" "Reasonable, but this is concise for now." >/dev/null 2>&1
seal_as "swift-falcon" threads resolve "$T_BUILDER" --reason "Agreed to defer" >/dev/null 2>&1
...
seal_as "quiet-owl" reviews abandon "$R3" --reason "Superseded by async server design" >/dev/null 2>&1
...
echo "Try:" >&2
echo "  cd $DEMO_DIR" >&2
echo "  seal --agent demo-viewer --scm git reviews list" >&2
echo "  seal --agent demo-viewer --scm git review $R1" >&2
echo "  seal --agent bold-tiger --scm git inbox" >&2
echo "" >&2
